...
__global__ void neutron_calculus(curandState *state, float c, float c_c, float h, float* absorbed, int r, int b, int t){
	...
	atomicAdd(&t,1);
	break;
      } else if ((u = curand_uniform (state + pos_Thread)) < c_c / c) {
	atomicAdd(&b,1);
...
int main(int argc, char *argv[]) {
  ...
  dim3 nbBlocks(n/threadsParBloc.x,1,1);
  float* absorbed_gpu;
  curandState* d_state;
  cudaMalloc(&absorbed_gpu, n*sizeof(float));
  start = my_gettimeofday();
  neutron_calculus<<<nbBlocks, threadsParBloc >>>(d_state, c, c_c, h, absorbed_gpu, r, b, t);
